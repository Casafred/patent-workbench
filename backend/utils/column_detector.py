...
class ColumnDetector:
    ...
    def __init__(self):
        ...
        self.patent_number_patterns = [
            # 中国专利号格式 - 支持更多位数变化
            r'CN\d{8,20}[A-Z]?\d?',     # CN202310123456A, CN202402107869A1 (8-20位数字)
            r'ZL\d{8,15}\.?\d?',        # ZL202310123456.7 (支持可选小数点)
            r'\d{4}\d{6,15}\.?\d?',     # 202310123456.7 (年份+6-15位数字)
            
            # 美国专利号格式 - 支持更宽泛的格式
            r'US\d{6,20}[A-Z]?\d?',     # US202402107869A1, US10123456B2 (6-20位数字)
            r'US\d{4,10}[A-Z]\d?',      # US1234A1 (较短格式)
            
            # 欧洲专利号格式
            r'EP\d{6,20}[A-Z]?\d?',     # EP1234567A1, EP202402107869A1
            
            # 日本专利号格式
            r'JP\d{4}-?\d{6,15}[A-Z]?', # JP2023-123456A, JP2024123456A (可选横线)
            r'JP\d{8,20}[A-Z]?\d?',     # JP202402107869A1
            r'特願\d{4}-?\d{6,12}',      # 特願2023-123456
            
            # 韩国专利号格式
            r'KR\d{2,4}-?\d{6,15}',     # KR10-1234567, KR20-202402107869
            r'KR\d{8,20}[A-Z]?\d?',     # KR202402107869A1
            
            # 德国专利号格式
            r'DE\d{6,20}[A-Z]?\d?',     # DE202402107869A1
            
            # 法国专利号格式
            r'FR\d{6,20}[A-Z]?\d?',     # FR202402107869A1
            
            # 英国专利号格式
            r'GB\d{6,20}[A-Z]?\d?',     # GB202402107869A1
            
            # 加拿大专利号格式
            r'CA\d{6,20}[A-Z]?\d?',     # CA202402107869A1
            
            # 澳大利亚专利号格式
            r'AU\d{6,20}[A-Z]?\d?',     # AU202402107869A1
            
            # 纯数字格式 - 支持各种长度
            r'\d{6,20}',                # 6-20位纯数字
            
            # 通用格式 - 更宽松的匹配
            r'[A-Z]{2,4}\d{6,20}[A-Z]?\d?',      # 2-4字母+6-20数字+可选字母数字
            r'[A-Z]{2,4}\d{4}-?\d{6,15}[A-Z]?',  # 2-4字母+年份-数字+字母
            r'[A-Z0-9]{8,25}',                   # 8-25位字母数字组合
            
            # 特殊格式支持
            r'[A-Z]{1,4}\d{6,20}[A-Z]{0,3}\d{0,2}',  # 灵活的字母数字组合
        ]
    ...
    def detect_patent_number_column(self, df: pd.DataFrame) -> Optional[Dict]:
        ...
        if results:
            ...
            return results[0] if results[0]['score'] >= 15 else None  # 降低阈值从20到15
