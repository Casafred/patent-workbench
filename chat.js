...
async function handleChatFileUpload(event, fileFromReuse = null) {
    ...
    try {
        ...
        if (fileFromReuse && fileFromReuse.id) {
            // 复用旧文件
            const contentResult = await apiCall(`/files/${fileFromReuse.id}/content`, null, 'GET');
...
async function handleStreamChatRequest() {
    ...
    // 如果有附加文件，添加文件内容到用户输入
    if (appState.chat.activeFile) {
        finalUserInput += `\n\n[附加文件内容]: ${appState.chat.activeFile.content}`;
    }
        ...
        <div class="message-main-content">
            ...
            <div class="message-body">
                ...
                <div class="message-footer">
                    ...
                    ${role === 'user' ? `
<button class="icon-button" title="重新发送" onclick="resendMessage(this)">
