...
def get_access_token(consumer_key, consumer_secret):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"✓ 成功获取 Token: {access_token[:30]}...")
            return access_token
        else:
            print(f"✗ 获取 Token 失败: {response.text[:500]}")
            return None
...
def test_biblio_epodoc(token, patent_number):
    """使用 epodoc 格式获取 biblio"""
    print("\n" + "="*60)
    print(f"2. 测试 biblio (epodoc 格式): {patent_number}")
    print("="*60)
...
def test_claims_epodoc(token, patent_number):
    """使用 epodoc 格式获取 claims"""
    print("\n" + "="*60)
    print(f"3. 测试 claims (epodoc 格式): {patent_number}")
    print("="*60)
...
def test_images_epodoc(token, patent_number):
    """使用 epodoc 格式获取 images"""
    print("\n" + "="*60)
    print(f"4. 测试 images (epodoc 格式): {patent_number}")
    print("="*60)
