...
所有模型都通过 **OpenAI兼容接口** 调用，无需学习不同API：
...
| 类型 | 说明 | 可否关闭 | 代表模型 |
|------|------|---------|---------|
| **混合思考模式** | 通过参数控制开关 | ✅ 可关闭 | qwen-plus, deepseek-v3.2, glm-5 |
| **仅思考模式** | 始终开启思考 | ❌ 不可关闭 | qwq-plus, deepseek-r1, kimi-k2-thinking |
| **非思考模式** | 不支持思考 | - | qwen-vl, qwen-coder, minimax-text-01 |
...
| 模型系列 | 思考模式 | 默认状态 | 特殊参数 |
|---------|---------|---------|---------|
| Qwen3.5 Plus/Flash | 混合思考 | 默认开启 | `enable_thinking`, `thinking_budget` |
| Qwen3 Max/Plus/Flash | 混合思考 | 默认关闭 | `enable_thinking`, `thinking_budget` |
| Qwen3 开源版 | 混合思考 | 默认开启 | 支持提示词 `/think` `/no_think` |
| QwQ-Plus/32B | 仅思考 | 始终开启 | 无需参数 |

```python
# 通义千问思考模式调用
completion = client.chat.completions.create(
    model="qwen-plus",
    messages=[{"role": "user", "content": "分析这个专利"}],
    extra_body={
        "enable_thinking": True,
        "thinking_budget": 100  # 限制思考过程最多100 Token
    },
    stream=True
)
```
...
| 模型 | 思考模式 | 默认状态 | 特点 |
|------|---------|---------|------|
| deepseek-v3.2 | 混合思考 | 默认关闭 | 最新模型，支持Function Calling |
| deepseek-v3.2-exp | 混合思考 | 默认关闭 | 实验版本 |
| deepseek-r1 | 仅思考 | 始终开启 | 深度推理，16K上下文 |
| deepseek-r1-distill-* | 仅思考 | 始终开启 | 蒸馏版，更经济 |
...
for chunk in completion:
    if not chunk.choices:
        # Token消耗统计
        print(f"思考Token: {chunk.usage.completion_tokens_details.reasoning_tokens}")
        continue
...
| 模型 | 思考模式 | 默认状态 | 特点 |
|------|---------|---------|------|
| glm-5 | 混合思考 | 默认开启 | 旗舰模型 |
| glm-4.7 | 混合思考 | 默认开启 | 平衡版 |
| glm-4.6 | 混合思考 | 默认开启 | - |
| glm-4.5 | 混合思考 | 默认开启 | 性价比版 |
| glm-4.5-air | 混合思考 | 默认开启 | 轻量版 |

```python
# GLM思考模式调用（支持thinking_budget）
completion = client.chat.completions.create(
    model="glm-5",
...
| 模型 | 思考模式 | 特点 |
|------|---------|------|
| kimi-k2.5 | 非思考 | 通用模型，1万亿参数MoE |
| kimi-k2-thinking | 仅思考 | 推理模型，始终开启思考 |
...
completion = client.chat.completions.create(
    ...
    extra_body={"thinking_budget": 100},  # 支持限制思考长度
    stream=True
)
```
...
| 模型 | 思考模式 | 特点 |
|------|---------|------|
| minimax-text-01 | 非思考 | 245K上下文，456B参数MoE |

**注意**: MiniMax模型当前仅提供免费体验，免费额度用完后不可调用且不支持付费。
...
for chunk in stream:
    if not chunk.choices:
        ...
        if hasattr(chunk, 'usage') and chunk.usage:
            ...
            print(f"思考Token消耗: {reasoning_tokens}")
        continue
...
| 功能 | Qwen | DeepSeek | GLM | Kimi | MiniMax |
|------|------|----------|-----|------|---------|
| 思考模式 | ✅ | ✅ | ✅ | ✅(仅K2-Thinking) | ❌ |
| 多轮对话 | ✅ | ✅ | ✅ | ✅ | ✅ |
| Function Calling | ✅ | ✅ | ✅ | ✅ | ❌ |
...
| 场景 | 推荐模型 | 原因 |
|------|---------|------|
| 日常对话 | qwen-plus / deepseek-v3.2 | 性价比高，可开关思考 |
| 复杂推理 | qwq-plus / deepseek-r1 | 仅思考模式，深度推理 |
| 专利分析 | qwen-plus (开启思考) | 平衡性能与成本 |
| 批量处理 | qwen-flash / deepseek-v3.2(关闭思考) | 高速低价 |
| 长文档处理 | qwen-long / minimax-text-01 | 超长上下文 |
...
- DeepSeek模型指南: https://help.aliyun.com/zh/dashscope/developer-reference/quick-start-for-deepseek-series-models
- 深度思考模型用法: https://help.aliyun.com/zh/model-studio/qwq
