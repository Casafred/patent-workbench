        ...
        return `
            ...
            window.copySectionContent = function(event, sectionId, sectionName) {
                ...
                navigator.clipboard.writeText(textToCopy).then(() => {
                    ...
                    if (btn) {
                        ...
                        btn.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 16 16" style="width:14px;height:14px"><path d="M10.97 4.97a.75.75 0 0 1 1.07 1.05l-3.99 4.99a.75.75 0 0 1-1.08.02L4.324 8.384a.75.75 0 1 1 1.06-1.06l2.094 2.093 3.473-4.425a.267.267 0 0 1 .02-.022z"/></svg> å·²å¤å?;
                        setTimeout(() => { btn.innerHTML = originalHTML; }, 2000);
                    }
                }).catch(err => {
                    console.error('å¤å¶å¤±è´¥:', err);
                    alert('å¤å¶å¤±è´¥ï¼è¯·æå¨å¤å¶');
                });
            };
            ...
            window.copyPatentNumbersListFromTab = function(event, tabId) {
                ...
                navigator.clipboard.writeText(textToCopy).then(() => {
                    ...
                    if (btn) {
                        const originalHTML = btn.innerHTML;
                        btn.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 16 16" style="width:14px;height:14px"><path d="M10.97 4.97a.75.75 0 0 1 1.07 1.05l-3.99 4.99a.75.75 0 0 1-1.08.02L4.324 8.384a.75.75 0 1 1 1.06-1.06l2.094 2.093 3.473-4.425a.267.267 0 0 1 .02-.022z"/></svg> å·²å¤å?' + patentNumbers.length + ' ä¸?;
                        setTimeout(() => { btn.innerHTML = originalHTML; }, 2000);
                    }
                }).catch(err => {
                    console.error('å¤å¶å¤±è´¥:', err);
            ...
            window.analyzeRelationPatents = function(event, patentNumber, relationType) {
                ...
                if (window.opener && window.opener.openRelationAnalysisTab) {
                    ...
                    alert('æ æ³è¿æ¥å°ä¸»çªå£ï¼è¯·ç¡®ä¿ä»ä¸»é¡µé¢æå¼æ­¤è¯¦æé¡µ');
                }
            };
            ...
            window.jumpToFamilyComparisonFromNewTab = function(event, patentNumber) {
                ...
                if (familyPatentNumbers.length < 2) {
                    alert('åæä¸å©æ°éä¸è¶³ï¼éè¦è³å°?ä¸ªåæä¸å©æè½è¿è¡å¯¹æ¯åæ?);
                    return;
                }
                if (window.opener && window.opener.startFamilyClaimsComparison) {
                    ...
                    alert('å·²è·³è½¬å°ä¸»é¡µé¢åè½ååæå¯¹æ¯åæçé¢ï¼è¯·å¨ä¸»é¡µé¢æ¥ç');
                } else {
                    alert('æ æ³è¿æ¥å°ä¸»çªå£çåæå¯¹æ¯åè½ï¼è¯·ç¡®ä¿ä»ä¸»é¡µé¢æå¼æ­¤è¯¦æé¡µï¼å¹¶å·æ°ä¸»é¡µé¢åéè¯');
                }
            };
            ...
            window.toggleDualColumnMode = function() {
                ...
                if (dualColumnMode) {
                    ...
                    btn.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" fill="currentColor" viewBox="0 0 16 16"><path d="M0 3a2 2 0 0 1 2-2h12a2 2 0 0 1 2 2v10a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2V3zm2-1a1 1 0 0 0-1 1v1h2V2H2zm3 2h2V2H5v2zm3-2v2h2V2H8zm3 2v2h2V4h-2zm0 3v2h2V7h-2zm0 3v2h2v-2h-2zm-3 2v2h2v-2H8zm-3 2v2h2v-2H5zm-3-2v2h2v-2H2zm0-3v2h2V7H2zm0-3v2h2V4H2zm5 0v2h2V4H7zm2 3H7v2h2V7z"/></svg> åæ å¯¹ç§';
            ...
            window.toggleImageTextMode = function() {
                ...
                if (drawings.length === 0) {
                    alert('å½åä¸å©æ²¡æéå¾ï¼æ æ³ä½¿ç¨å¾æå¯¹ç§æ¨¡å¼?);
                    imageTextMode = false;
                    return;
                }
                ...
                if (imageTextMode) {
                    ...
                    toolbar.innerHTML = '<div style="display: flex; align-items: center; gap: 10px;"><span id="image-text-counter" style="font-weight: 500; font-size: 14px;">å?1 / ' + drawings.length + '</span></div><div style="display: flex; gap: 8px; align-items: center;"><button onclick="imageTextZoomIn()" title="æ¾å¤§" style="background: rgba(255,255,255,0.2); border: none; color: white; width: 32px; height: 32px; border-radius: 6px; cursor: pointer; font-size: 18px;">+</button><span id="image-text-zoom" style="min-width: 50px; text-align: center; font-size: 13px;">100%</span><button onclick="imageTextZoomOut()" title="ç¼©å°" style="background: rgba(255,255,255,0.2); border: none; color: white; width: 32px; height: 32px; border-radius: 6px; cursor: pointer; font-size: 18px;">-</button><button onclick="imageTextRotateLeft()" title="åå·¦æè½¬" style="background: rgba(255,255,255,0.2); border: none; color: white; width: 32px; height: 32px; border-radius: 6px; cursor: pointer; font-size: 16px;">â?/button><button onclick="imageTextRotateRight()" title="åå³æè½¬" style="background: rgba(255,255,255,0.2); border: none; color: white; width: 32px; height: 32px; border-radius: 6px; cursor: pointer; font-size: 16px;">â?/button><button onclick="imageTextSmartMarker()" title="æºè½æ è®°" style="background: linear-gradient(135deg, #28a745 0%, #20c997 100%); border: none; color: white; padding: 6px 12px; border-radius: 6px; cursor: pointer; font-size: 13px; font-weight: 500;">æºè½æ è®°</button></div>';
                    ...
                    btn.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" fill="currentColor" viewBox="0 0 16 16"><path d="M.002 3a2 2 0 0 1 2-2h12a2 2 0 0 1 2 2v10a2 2 0 0 1-2 2h-12a2 2 0 0 1-2-2V3zm1 9v1a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1V9.5l-3.777-1.947a.5.5 0 0 0-.577.093l-3.71 3.71-2.66-1.772a.5.5 0 0 0-.63.062L1.002 12zm5-6.5a1.5 1.5 0 1 0-3 0 1.5 1.5 0 0 0 3 0z"/></svg> å¾æå¯¹ç§';
            ...
            function imageTextUpdateDisplay() {
                ...
                if (counter) counter.textContent = 'å?' + (imageTextViewerIndex + 1) + ' / ' + drawings.length;
            ...
            window.imageTextSmartMarker = function() {
                ...
                if (window.opener && !window.opener.closed) {
                    ...
                    if (typeof window.opener.sendToDrawingMarker === 'function') {
                        ...
                        window.opener.focus();
                        alert('å·²ä¼ éæ°æ®å°ä¸»é¡µé¢ï¼è¯·å¨ä¸»é¡µé¢ä¸­éæ©è¦æ è®°çå¾ç');
                        return;
                    }
                }
                
                alert('è¯·å¨ä¸»é¡µé¢ä¸­ä½¿ç¨æ­¤åè½ï¼æç¡®ä¿ä¸»é¡µé¢å·²å è½½å®æ?);
            };
            ...
            window.openNewTabImageViewer = function(startIndex) {
                ...
                var viewerHTML = '<div id="image-viewer-overlay" style="position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.92); z-index: 10000; display: flex; flex-direction: column; align-items: center; justify-content: center;">' +
                    ...
                    '<span id="viewer-counter" style="color: white; font-size: 18px; font-weight: 500;">å?' + (viewerIndex + 1) + ' / ' + drawings.length + '</span>' +
                    ...
                    '<button onclick="zoomNewTabImage(-0.2)" style="background: rgba(255,255,255,0.2); border: none; color: white; font-size: 28px; width: 56px; height: 56px; border-radius: 50%; cursor: pointer;">-</button>' +
                    '<button onclick="rotateNewTabImage(-90)" style="background: rgba(255,255,255,0.2); border: none; color: white; font-size: 22px; width: 56px; height: 56px; border-radius: 50%; cursor: pointer;">â?/button>' +
                    '</div>' +
                    '<div style="position: absolute; right: 30px; top: 50%; transform: translateY(-50%); display: flex; flex-direction: column; gap: 8px;">' +
                    '<button onclick="navigateNewTabViewer(1)" style="background: rgba(255,255,255,0.2); border: none; color: white; font-size: 36px; width: 56px; height: 56px; border-radius: 50%; cursor: pointer;">&#8250;</button>' +
                    '<button onclick="rotateNewTabImage(90)" style="background: rgba(255,255,255,0.2); border: none; color: white; font-size: 22px; width: 56px; height: 56px; border-radius: 50%; cursor: pointer;">â?/button>' +
                    '<button onclick="sendNewTabDrawingsToMarker()" style="background: linear-gradient(135deg, #28a745 0%, #20c997 100%); border: none; color: white; font-size: 12px; width: 56px; height: 56px; border-radius: 50%; cursor: pointer; font-weight: bold; line-height: 1.1; text-align: center;">æºè½<br>æ è®°</button>' +
                    '</div>' +
            ...
            function updateViewerImage() {
                ...
                if (counter) counter.textContent = 'å?' + (viewerIndex + 1) + ' / ' + drawings.length;
            ...
            window.showTranslateDialogNewTab = function(event, textType, column) {
                ...
                dialog.innerHTML = '<div class="drag-handle" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px; cursor: move;">' +
                    ...
                    '</div>' +
                    (cachedModel ? '<p style="margin: 0 0 8px 0; color: #28a745; font-size: 12px;">å·²æç¼å­ (æ¨¡å: ' + cachedModel + ')</p>' : '') +
                    ...
                    '<button onclick="document.getElementById(\\'translate-dialog-newtab\\').remove()" style="flex: 1; padding: 8px; border: 1px solid #ddd; background: white; border-radius: 6px; cursor: pointer;">åæ¶</button>' +
            ...
            window.startTranslationNewTab = async function(textType, model) {
                ...
                if (btn) {
                    btn.disabled = true;
                    btn.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" fill="currentColor" viewBox="0 0 16 16" style="animation: spin 1s linear infinite;"><path d="M8 3a5 5 0 1 0 4.546 2.914.5.5 0 0 1 .908-.417A6 6 0 1 1 8 2v1z"/></svg> ç¿»è¯ä¸?..';
                }
                ...
                if (cached) {
                    try {
                        ...
                        if (Date.now() - data.timestamp < 7 * 24 * 60 * 60 * 1000) {
                            ...
                            if (btn) {
                                btn.disabled = false;
                                btn.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" fill="currentColor" viewBox="0 0 16 16"><path d="M0 8a8 8 0 1 1 16 0A8 8 0 0 1 0 8zm7.5-6.923c-.67.204-1.335.82-1.887 1.855A7.97 7.97 0 0 0 5.145 4H7.5V1.077zM4.09 4a9.267 9.267 0 0 1 .64-1.539 6.7 6.7 0 0 1 .597-.933A7.025 7.025 0 0 0 2.255 4H4.09zm-.582 3.5c.03-.877.138-1.718.312-2.5H1.674a6.958 6.958 0 0 0-.656 2.5h2.49zM4.847 5a12.5 12.5 0 0 0-.338 2.5H7.5V5H4.847zM8.5 5v2.5h2.99a12.495 12.495 0 0 0-.337-2.5H8.5zM4.51 8.5a12.5 12.5 0 0 0 .337 2.5H7.5V8.5H4.51zm3.99 0V11h2.653c.187-.765.306-1.608.338-2.5H8.5zM5.145 12c.138.386.295.744.468 1.068.552 1.035 1.218 1.65 1.887 1.855V12H5.145zm.182 2.472a6.696 6.696 0 0 1-.597-.933A9.268 9.268 0 0 1 4.09 12H2.255a7.024 7.024 0 0 0 3.072 2.472zM3.82 11a13.652 13.652 0 0 1-.312-2.5h-2.49A6.95 6.95 0 0 0 1.674 11H3.82zm10.026-2.5a13.65 13.65 0 0 1-.312 2.5h2.146c.22-.765.368-1.608.426-2.5h-2.26zm-1.068 2.5c-.138.386-.295.744-.468 1.068-.552 1.035-1.218 1.65-1.887 1.855V12h2.355zm.182 2.472A6.696 6.696 0 0 0 13.91 12h1.835a7.024 7.024 0 0 1-3.072 2.472zM14.326 11a6.95 6.95 0 0 0 .656-2.5h-2.49c-.03.877-.138 1.718-.312 2.5h2.146z"/></svg> ç¿»è¯';
                            }
                            return;
                        }
                    } catch(e) {}
                ...
                try {
                    ...
                    if (!zhipuKey && !aliyunKey) {
                        throw new Error('è¯·åéç½®API Key');
                    }
                    ...
                    if (textType === 'claims') {
                        ...
                        var response = await fetch('/api/chat', {
                            ...
                            body: JSON.stringify({
                                model: model,
                                messages: [
                                    { role: 'system', content: 'ä½ æ¯ä¸ä½ä¸ä¸çä¸å©æç®ç¿»è¯ä¸å®¶ãè¯·å°ä»¥ä¸è±æä¸å©æå©è¦æ±ç¿»è¯ä¸ºä¸­æãä¿æä¸å©æ¯è¯­çåç¡®æ§ï¼ä¿çæææ°å­æ è®°ï¼ç¿»è¯è¦æµçèªç¶ãä¿ææå©è¦æ±çç¼å·åæ ¼å¼ãåªè¿åç¿»è¯ç»æï¼ä¸è¦æ·»å ä»»ä½è§£éãè¯·æç§ä»¥ä¸æ ¼å¼è¿åï¼æå©è¦æ±?1: [ç¿»è¯åå®¹]' },
                        ...
                        if (!response.ok) {
                            ...
                            throw new Error(errorData.error && errorData.error.message ? errorData.error.message : (errorData.error || 'APIè¯·æ±å¤±è´¥: ' + response.status));
                        }
                        ...
                        var pattern = /æå©è¦æ±\s*(\d+)[:ï¼]\s*(.*?)(?=æå©è¦æ±\s*\d+[:ï¼]|$)/gs;
                        ...
                        if (matches.length > 0) {
                            ...
                            claims.forEach(function(claim, i) {
                                ...
                                translations.push({
                                    original: claimText,
                                    translated: translatedMap[i + 1] || '[ç¿»è¯è§£æå¤±è´¥]',
                                    index: i + 1
                                });
                            });
                        } else {
                        ...
                        if (!description) throw new Error('æ²¡æå¯ç¿»è¯çè¯´æä¹¦åå®?);
                        ...
                        var response = await fetch('/api/chat', {
                            ...
                            body: JSON.stringify({
                                model: model,
                                messages: [
                                    { role: 'system', content: 'ä½ æ¯ä¸ä½ä¸ä¸çä¸å©æç®ç¿»è¯ä¸å®¶ãè¯·å°ä»¥ä¸è±æä¸å©è¯´æä¹¦ç¿»è¯ä¸ºä¸­æãä¿æä¸å©æ¯è¯­çåç¡®æ§ï¼ä¿çæææ°å­æ è®°ï¼ç¿»è¯è¦æµçèªç¶ãåªè¿åç¿»è¯ç»æï¼ä¸è¦æ·»å ä»»ä½è§£éã? },
                    ...
                    console.log('ç¿»è¯ç»æå·²ç¼å­?, cacheKey);
            ...
            window.showTranslationResultNewTab = function(translations, textType) {
                ...
                if (textType === 'claims' && translations[0] && translations[0].index) {
                    translations.forEach(function(t) {
                        content += '<div style="padding: 12px; border-bottom: 1px solid #eee;">' +
                            '<div style="font-weight: 600; color: #009688; margin-bottom: 6px;">æå©è¦æ± ' + t.index + '</div>' +
                            '<div style="font-size: 12px; color: #666; margin-bottom: 4px;">åæ:</div>' +
                ...
                panel.innerHTML = '<div class="drag-handle" style="display: flex; justify-content: space-between; align-items: center; padding: 12px 16px; background: linear-gradient(135deg, #00bcd4 0%, #009688 100%); color: white; cursor: move;">' +
                    '<h4 style="margin: 0; display: flex; align-items: center; gap: 8px;"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16"><path d="M0 8a8 8 0 1 1 16 0A8 8 0 0 1 0 8zm7.5-6.923c-.67.204-1.335.82-1.887 1.855A7.97 7.97 0 0 0 5.145 4H7.5V1.077zM4.09 4a9.267 9.267 0 0 1 .64-1.539 6.7 6.7 0 0 1 .597-.933A7.025 7.025 0 0 0 2.255 4H4.09zm-.582 3.5c.03-.877.138-1.718.312-2.5H1.674a6.958 6.958 0 0 0-.656 2.5h2.49zM4.847 5a12.5 12.5 0 0 0-.338 2.5H7.5V5H4.847zM8.5 5v2.5h2.99a12.495 12.495 0 0 0-.337-2.5H8.5zM4.51 8.5a12.5 12.5 0 0 0 .337 2.5H7.5V8.5H4.51zm3.99 0V11h2.653c.187-.765.306-1.608.338-2.5H8.5zM5.145 12c.138.386.295.744.468 1.068.552 1.035 1.218 1.65 1.887 1.855V12H5.145zm.182 2.472a6.696 6.696 0 0 1-.597-.933A9.268 9.268 0 0 1 4.09 12H2.255a7.024 7.024 0 0 0 3.072 2.472zM3.82 11a13.652 13.652 0 0 1-.312-2.5h-2.49A6.95 6.95 0 0 0 1.674 11H3.82zm10.026-2.5a13.65 13.65 0 0 1-.312 2.5h2.146c.22-.765.368-1.608.426-2.5h-2.26zm-1.068 2.5c-.138.386-.295.744-.468 1.068-.552 1.035-1.218 1.65-1.887 1.855V12h2.355zm.182 2.472A6.696 6.696 0 0 0 13.91 12h1.835a7.024 7.024 0 0 1-3.072 2.472zM14.326 11a6.95 6.95 0 0 0 .656-2.5h-2.49c-.03.877-.138 1.718-.312 2.5h2.146z"/></svg> ç¿»è¯ç»æ - ' + (textType === 'claims' ? 'æå©è¦æ±' : 'è¯´æä¹?) + '</h4>' +
                    ...
                    '<button onclick="replaceContentWithTranslation(\\'' + textType + '\\')" style="padding: 6px 12px; border: none; background: linear-gradient(135deg, #2e7d32 0%, #43a047 100%); color: white; border-radius: 6px; cursor: pointer; font-size: 12px;">æ¿æ¢åæ</button>' +
                    '<button onclick="document.getElementById(\\'' + panelId + '\\').remove()" style="padding: 6px 12px; border: 1px solid #ddd; background: white; border-radius: 6px; cursor: pointer; font-size: 12px;">å³é­</button>' +
                    '</div>';
                ...
                if (textType === 'claims') {
                    if (targetContainer) {
                        ...
                        if (claimsList) {
                            var newHtml = '';
                            translations.forEach(function(t) {
                                newHtml += '<div class="claim-item" data-claim-number="' + t.index + '"><div class="claim-number">æå©è¦æ± ' + t.index + '</div><div class="claim-text">' + escapeHtmlNewTab(t.translated) + '</div></div>';
                            });
            ...
            window.updateNewTabContextInfo = function() {
                ...
                if (checkbox.checked) {
                    ...
                    infoSpan.textContent = 'çº?' + tokenCount + ' Token (' + charCount + ' å­ç¬¦)';
            ...
            function getApiKeysFromOpener() {
                ...
                if (window.opener) {
                    try {
                        ...
                        console.warn('æ æ³ä»ä¸»çªå£è·åAPI Key:', e);
                    }
                }
                return keys;
            }
            ...
            async function initNewTabChatProviders() {
                ...
                try {
                    ...
                    console.warn('å è½½æå¡åéç½®å¤±è´¥ï¼ä½¿ç¨é»è®¤éç½®');
            ...
            function updateNewTabChatProviderSelect() {
                ...
                for (var i = 0; i < providerKeys.length; i++) {
                    ...
                    var label = hasKey ? val.name : val.name + ' (æªéç½?';
            ...
            function createNewTabChatModal(patentNumber, patentData) {
                ...
                chatModal.innerHTML = '<div style="background: white; border-radius: 12px; width: 90%; max-width: 800px; max-height: 85vh; display: flex; flex-direction: column; box-shadow: 0 8px 32px rgba(0,0,0,0.3);">' +
                    ...
                    '<div><h4 style="margin: 0; font-size: 18px; color: white;">ä¸å©å¯¹è¯ï¼? + patentNumber + '</h4>' +
                    '<p style="margin: 4px 0 0 0; font-size: 13px; color: rgba(255,255,255,0.85);">' + (patentData.title || 'æ æ é¢?) + '</p></div>' +
                    '<div style="display: flex; gap: 8px;">' +
                    '<button onclick="minimizeNewTabChat()" title="æå°åä¸ºæ¬æµ®ç" style="background: rgba(255,255,255,0.2); border: none; font-size: 18px; cursor: pointer; color: white; width: 32px; height: 32px; border-radius: 50%; display: flex; align-items: center; justify-content: center;">â?/button>' +
                    ...
                    '<div style="display: flex; align-items: center; gap: 8px;"><label style="font-size: 13px; color: #2e7d32; font-weight: 500;">æå¡å?</label>' +
                    ...
                    '<div style="display: flex; align-items: center; gap: 8px;"><label style="font-size: 13px; color: #2e7d32; font-weight: 500;">æ¨¡å:</label>' +
                    ...
                    '<label class="checkbox-label" title="å¾éåå°åå«å®æ´çè¯´æä¹¦åæå©è¦æ±ä½ä¸ºä¸ä¸æ? style="display: inline-flex; align-items: center; gap: 6px; cursor: pointer; font-size: 13px; color: #2e7d32;"><input type="checkbox" id="newtab_chat_full_context" onchange="updateNewTabContextInfo()"><span>åå«å®æ´åå®¹</span></label>' +
                    ...
                    '<button onclick="clearNewTabChatHistory()" style="padding: 6px 12px; background: white; border: 1px solid #c8e6c9; border-radius: 6px; font-size: 13px; color: #2e7d32; cursor: pointer;">æ¸ç©ºå¯¹è¯</button>' +
                    '</div></div>' +
                    ...
                    '<p style="font-size: 16px; margin: 0;">æ¨å¥½ï¼ææ¯ä¸å©æºè½å©æã?/p>' +
                    '<p style="font-size: 14px; color: #999; margin-top: 8px;">å¨ä¸æ¹è¾å¥æ¨çé®é¢ï¼å¼å§ä¸AIå¯¹è¯</p>' +
                    '</div></div>' +
                    '<div style="padding: 16px; background: white; border-top: 1px solid #e8f5e9;">' +
                    '<div style="display: flex; gap: 10px;">' +
                    '<textarea id="newtab_chat_input" placeholder="è¾å¥æ¨çé®é¢... (Shift+Enteræ¢è¡ï¼Enteråé?" style="flex: 1; padding: 12px 16px; border: 2px solid #c8e6c9; border-radius: 12px; font-size: 14px; resize: none; height: 48px; line-height: 1.4;"></textarea>' +
                    '<button id="newtab_chat_send_btn" onclick="sendNewTabPatentChatMessage()" style="background: linear-gradient(135deg, #2e7d32 0%, #43a047 100%); color: white; border: none; padding: 12px 24px; border-radius: 12px; font-size: 14px; cursor: pointer; font-weight: 500;">åé?/button>' +
                    '<button id="newtab_chat_stop_btn" onclick="stopNewTabChatStream()" style="background: #ef5350; color: white; border: none; padding: 12px 24px; border-radius: 12px; font-size: 14px; cursor: pointer; font-weight: 500; display: none;">åæ­¢</button>' +
                    '</div>' +
                    '<div style="margin-top: 10px; display: flex; gap: 8px; flex-wrap: wrap;">' +
                    '<button onclick="document.getElementById(\\'newtab_chat_input\\').value=\\'è¿ä¸ªä¸å©çæ ¸å¿ææ¯æ¯ä»ä¹ï¼\\'" style="background: #e8f5e9; border: none; padding: 6px 12px; border-radius: 15px; font-size: 12px; cursor: pointer; color: #2e7d32;">æ ¸å¿ææ?/button>' +
                    '<button onclick="document.getElementById(\\'newtab_chat_input\\').value=\\'è¿ä¸ªä¸å©çåæ°ç¹å¨åªéï¼\\'" style="background: #e8f5e9; border: none; padding: 6px 12px; border-radius: 15px; font-size: 12px; cursor: pointer; color: #2e7d32;">åæ°ç?/button>' +
                    '<button onclick="document.getElementById(\\'newtab_chat_input\\').value=\\'è¯·è§£éä¸ä¸æå©è¦æ±?\\'" style="background: #e8f5e9; border: none; padding: 6px 12px; border-radius: 15px; font-size: 12px; cursor: pointer; color: #2e7d32;">è§£éæå©è¦æ±</button>' +
                    '<button onclick="document.getElementById(\\'newtab_chat_input\\').value=\\'è¿ä¸ªä¸å©çåºç¨åºæ¯æåªäºï¼\\'" style="background: #e8f5e9; border: none; padding: 6px 12px; border-radius: 15px; font-size: 12px; cursor: pointer; color: #2e7d32;">åºç¨åºæ¯</button>' +
                    '</div></div></div>';
            ...
            window.closeNewTabPatentChat = function() {
                ...
                if (nonSystemMessages.length >= 2 && window.opener && !window.opener.closed) {
                    var shouldSync = confirm('æ¯å¦å°æ¬æ¬¡å¯¹è¯è®°å½åæ­¥å°ä¸»é¡µé¢åå²ï¼\\n\\nåæ­¥åå¯å¨ä¸»é¡µé¢"åè½ä¸å³æ¶å¯¹è¯"ä¸­æ¥çåç»§ç»­æ­¤å¯¹è¯ã?);
                    ...
                    if (shouldSync) {
                        try {
                            if (window.opener.ChatHistorySync) {
                                ...
                                alert('å¯¹è¯å·²åæ­¥å°ä¸»é¡µé¢åå²è®°å½ï¼');
                            } else {
                                alert('ä¸»é¡µé¢æªå è½½å¯¹è¯åæ­¥æ¨¡åï¼æ æ³åæ­¥ã?);
                            }
                        } catch (e) {
                            console.error('åæ­¥å¯¹è¯å¤±è´¥:', e);
                            alert('åæ­¥å¤±è´¥ï¼è¯·ç¡®ä¿ä¸»é¡µé¢å·²å è½½å®æã?);
                        }
                    }
                }
            ...
            window.clearNewTabChatHistory = function() {
                var historyEl = document.getElementById('newtab_chat_history');
                historyEl.innerHTML = '<div class="welcome-message" style="text-align: center; padding: 40px 20px; color: #666;"><div style="font-size: 48px; margin-bottom: 16px;">ð¬</div><p style="font-size: 16px; margin: 0;">ææ å¯¹è¯è®°å½</p><p style="font-size: 14px; color: #999; margin-top: 8px;">å¨ä¸æ¹è¾å¥æ¨çé®é¢ï¼å¼å§ä¸AIå¯¹è¯</p></div>';
                window.newTabChatState.messages = [];
            };
            ...
            window.sendNewTabPatentChatMessage = async function() {
                ...
                aiMsgDiv.innerHTML = '<div style="max-width: 70%; background: white; padding: 12px 16px; border-radius: 16px 16px 16px 4px; box-shadow: 0 2px 8px rgba(0,0,0,0.1); border: 1px solid #e8f5e9;"><div style="font-size: 14px; color: #666;">æèä¸­...</div></div>';
                ...
                try {
                    ...
                    var safeValue = function(val) {
                        if (!val) return 'æªç¥';
                        if (Array.isArray(val)) return val.length > 0 ? val.join(', ') : 'æªç¥';
                        return String(val);
                    };
                    
                    var safeArray = function(val, limit) {
                        limit = limit || 10;
                        if (!val || !Array.isArray(val) || val.length === 0) return 'æ?;
                        return val.slice(0, limit).map(function(item, i) { return (i + 1) + '. ' + (typeof item === 'string' ? item : JSON.stringify(item)); }).join('\\n');
                    };
                    
                    var contextInfo = 'ä½ æ¯ä¸ä¸ªä¸ä¸çä¸å©åæå©æãå½åæ­£å¨åæä¸å©å·ä¸?' + patentNumber + ' çä¸å©ãè¯·åºäºä»¥ä¸å®æ´çä¸å©ä¿¡æ¯ï¼åç¡®ãä¸ä¸å°åç­ç¨æ·çé®é¢ã\\n\\n';
                    contextInfo += '## ä¸å©åºæ¬ä¿¡æ¯\\n';
                    contextInfo += '- **ä¸å©å?*: ' + (patentInfo.patent_number || patentNumber) + '\\n';
                    contextInfo += '- **æ é¢**: ' + (patentInfo.title || 'æ æ é¢?) + '\\n';
                    contextInfo += '- **ç³è¯·æ¥æ**: ' + (patentInfo.application_date || patentInfo.filing_date || 'æªç¥') + '\\n';
                    contextInfo += '- **å¬å¼æ¥æ**: ' + (patentInfo.publication_date || 'æªç¥') + '\\n';
                    contextInfo += '- **æææ¥æ**: ' + (patentInfo.grant_date || 'æªç¥') + '\\n';
                    contextInfo += '- **ä¼åææ¥æ?*: ' + (patentInfo.priority_date || 'æªç¥') + '\\n';
                    contextInfo += '- **æ³å¾ç¶æ?*: ' + (patentInfo.legal_status || 'æªç¥') + '\\n';
                    contextInfo += '\\n## ç³è¯·äººä¸åæäºº\\n';
                    contextInfo += '- **ç³è¯·äº?åè®©äº?*: ' + safeValue(patentInfo.assignees || patentInfo.applicant) + '\\n';
                    contextInfo += '- **åæäº?*: ' + safeValue(patentInfo.inventors || patentInfo.inventor) + '\\n';
                    contextInfo += '\\n## åç±»ä¿¡æ¯\\n';
                    contextInfo += '- **IPCåç±»**: ' + safeValue(patentInfo.ipc_classification) + '\\n';
                    contextInfo += '- **CPCåç±»**: ' + safeValue(patentInfo.cpc_classification) + '\\n';
                    ...
                    if (patentInfo.description) {
                        ...
                        if (useFullContext) {
                            ...
                            var truncatedDesc = descText.length > 5000 ? descText.substring(0, 5000) + '...(åå®¹è¿é¿å·²æªæ?' : descText;
                            contextInfo += '\\n## è¯´æä¹¦\\n' + truncatedDesc + '\\n';
                        }
                    }
                    ...
                    contextInfo += '\\nè¯·åºäºä»¥ä¸å®æ´çä¸å©ä¿¡æ¯ï¼åç¡®ãä¸ä¸å°åç­ç¨æ·çé®é¢ãåç­æ¶å¯ä»¥ä½¿ç¨Markdownæ ¼å¼æ¥ç»ç»åå®¹ï¼ä½¿å¶æ´æè¯»ã?;
                    ...
                    if (window.newTabChatState.currentProvider === 'aliyun') {
                        var aliyunKey = window.newTabChatState.apiKeys.aliyun;
                        if (!aliyunKey) throw new Error('è¯·åéç½®é¿éäºAPIå¯é¥');
                        headers['X-LLM-Provider'] = 'aliyun';
                        headers['Authorization'] = 'Bearer ' + aliyunKey;
                    } else {
                        var apiKey = window.newTabChatState.apiKeys.zhipu;
                        if (!apiKey) throw new Error('è¯·åéç½®æºè°±APIå¯é¥');
                    ...
                    contentDiv.innerHTML = '<span style="color: #c62828;">åéå¤±è´? ' + escapeHtmlNewTab(error.message) + '</span>';
                } finally {
                    window.newTabChatState.isLoading = false;
    ...
    window._openPatentDetailInNewTabImpl = function(patentNumber) {
        ...
        const htmlContent = `
            ...
            <head>
                ...
                <title>${U.safeStr(data.title) || patentNumber} - ä¸å©è¯¦æ</title>
            ...
            <body>
                <div class="nav-trigger" id="navTrigger" title="æ¬æµ®å±å¼å¯¼èªæ ?>
