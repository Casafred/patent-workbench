    ...
    function buildPageScripts(patentNumber, data) {
        ...
        return `
            ...
            window.exportPatentDetailToWordFromNewTab = function(patentNumber) {
                ...
                try {
                    const htmlContent = generateWordHtml(patentNumber, patentData, analysisResult);
            ...
            function generateWordHtml(patentNumber, patentData, analysisResult) {
                let html = '<!DOCTYPE html><html><head><meta charset="UTF-8"><style>' +
                    'body{font-family:"Microsoft YaHei","SimSun",Arial,sans-serif;font-size:12pt;line-height:1.6;color:#333;}' +
                    'h1{font-size:18pt;color:#2E7D32;text-align:center;margin-bottom:20pt;}' +
                    'h2{font-size:14pt;color:#2E7D32;border-bottom:1px solid #2E7D32;padding-bottom:5pt;margin-top:20pt;margin-bottom:10pt;}' +
                    'table{width:100%;border-collapse:collapse;margin:10pt 0;}' +
                    'th,td{border:1px solid #ccc;padding:8pt;text-align:left;}' +
                    'th{background-color:#2E7D32;color:white;font-weight:bold;}' +
                    '.label-cell{background-color:#f5f5f5;font-weight:bold;width:25%;}' +
                    '.translation-label{color:#009688;font-weight:bold;margin-top:10pt;}' +
                    '.claim-item{margin-bottom:15pt;}' +
                    '.claim-title{font-weight:bold;color:#2E7D32;}' +
                    '.claim-dependent{color:#1976D2;}' +
                    '.meta-info{margin-bottom:15pt;color:#666;}' +
                    '.section{margin-bottom:20pt;}' +
                    '</style></head><body>' +
                    '<h1>专利详情报告</h1>' +
                    '<div class="meta-info">' +
                    '<p><strong>专利号:</strong> ' + escapeHtml(patentNumber || '-') + '</p>';
                
                if (patentData.title) {
                    html += '<p><strong>标题:</strong> ' + escapeHtml(patentData.title) + '</p>';
                }
                html += '<p><strong>导出时间:</strong> ' + new Date().toLocaleString('zh-CN') + '</p></div>';
                
                html += '<h2>一、基本信息</h2><table>';
                if (patentData.inventors && patentData.inventors.length > 0) {
                    html += '<tr><td class="label-cell">发明人</td><td>' + escapeHtml(patentData.inventors.join(', ')) + '</td></tr>';
                }
                if (patentData.assignees && patentData.assignees.length > 0) {
                    html += '<tr><td class="label-cell">申请人</td><td>' + escapeHtml(patentData.assignees.join(', ')) + '</td></tr>';
                }
                if (patentData.application_date) {
                    html += '<tr><td class="label-cell">申请日期</td><td>' + escapeHtml(patentData.application_date) + '</td></tr>';
                }
                ...
                if (analysisResult) {
                    ...
                    try {
                        ...
                        html += '<p>' + escapeHtml(analysisResult.analysis_content) + '</p>';
                    }
                }
