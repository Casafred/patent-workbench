...
function initAsyncBatch() {
    ...
    addTemplateBtn.addEventListener('click', () => {
        ...
        if (!name || !user) return alert('模板名称和用户提示模板不能为空！');
        if (!user.includes('{{INPUT}}')) return alert('用户提示模板必须包含 {{INPUT}} 占位符！');
        appState.asyncBatch.templates.push({ id: `T${appState.asyncBatch.nextTemplateId++}`, name, systemPrompt: systemPromptInput.value.trim(), userPromptTemplate: user });
        renderAsyncLists();
...
function renderAsyncLists() {
    ...
    templatesListDiv.innerHTML = appState.asyncBatch.templates.map(t => `<div class="list-item"><span class="item-content"><strong>${t.id}:</strong> ${t.name}</span><button class="icon-button" title="删除" onclick="deleteAsyncTemplate('${t.id}')">🗑️</button></div>`).join('') || '<div class="info" style="padding:10px">暂无模板</div>';
...
async function submitTaskWithRetry(request, retries = MAX_ASYNC_RETRIES) {
    ...
    const body = { model: appState.asyncBatch.model, temperature: appState.asyncBatch.temperature, messages, request_id: request.localId };
